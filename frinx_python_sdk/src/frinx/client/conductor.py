...
class MetadataClient(BaseClient):
    ...
    def __init__(self, baseURL, headers=None):
        BaseClient.__init__(self, "http://127.0.0.1:/api/workflow", self.BASE_RESOURCE, headers)
...
def main():
    ...
    if command == "start":
        if len(sys.argv) < 7:
            print(
                "python conductor server_url start workflow_name input_json [version] [correlationId]"
            )
            return None
        wfName = sys.argv[3]
